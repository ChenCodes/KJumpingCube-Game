...
class Game extends Observable {
    ...
    int play() {
        ...
        while (_exit != 0) {
            try {
                ...
                reportError("syntax error in " + "'" + nextInput
                    + "'"  + " command");
            }
            if (_exit == 0) {
                _prompter.close();
     ...
     *  subsequent moves.
     * @param color This holds the color of my side.
     */
    ...
    private Side readSide() {
        ...
        if (_inp.hasNextInt()) {
            _inp.nextInt();
            _inp.nextLine();
            reportError("syntax error in " + "'" + "manual" + "'"
                +  " command");
        } else if (_inp.hasNext()) {
            currSide = _inp.next().toLowerCase();
            _inp.nextLine();
    ...
    outerloop:
        while (_playing) {
            if (_board.numOfSide(RED) == _board.size()
                * _board.size() || _board.numOfSide(BLUE)
            ...
            if (getPlayer(_board.whoseMove()) instanceof HumanPlayer) {
                ...
                if (_board.numOfSide(RED) == _board.size() * _board.size()
                    ||
                    _board.numOfSide(BLUE) == _board.size() * _board.size()) {
                    announceWinner();
                    restartGame();
                    break;
                }
    ...
    private void readExecuteCommand() {
        ...
        if (_inp.hasNextInt()) {
            ...
            } else if (_board.get(row, col).getSide()
    ...
    private void executeCommand(String cmnd) {
        ...
        case "size":
            ...
            if (_inp.hasNextInt()) {
                ...
                _inp.nextLine();
                reportError("syntax error in " + "'" + "size"
                    + "'" + " command");
                break;
            }
            break;
